...
import {
    listUsers, showUser, storeUser, updateUserController,
    toggleActive, destroyUser, addRoleToUser, removeUserRole,
...
router.post("/",      requireAuth, requirePermission("Create User"),  validateSchema(createUserSchema), storeUser);
router.put("/:id",    requireAuth, requirePermission("Update User"),  validateSchema(updateUserSchema), updateUserController);
...
router.post("/:id/roles",           requireAuth, requirePermission("Update User"), validateSchema(assignPermissionSchema), addRoleToUser);
