...
export const updatePermissionGroupController = async (
  request: Request,
  response: Response,
): Promise<void> => {
  await handleRequest(response, () =>
    updatePermissionGroup(Number(request.params.id), request.body),
  );
};

export const destroyPermissionGroup = async (
  request: Request,
  response: Response,
): Promise<void> => {
  await handleRequest(
