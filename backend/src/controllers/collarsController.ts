...
export const listCollars = async (
  _request: Request,
  response: Response,
): Promise<void> => {
...
export const showCollar = async (
  request: Request,
  response: Response,
): Promise<void> => {
  await handleRequest(
    response,
    () => getCollarById(Number(request.params.id)),
    200,
    404,
  );
};

export const storeCollar = async (
  request: Request,
  response: Response,
): Promise<void> => {
...
export const updateCollarController = async (
  ...
  await handleRequest(response, () =>
    updateCollar(Number(request.params.id), request.body),
  );
};

export const destroyCollar = async (
  request: Request,
  response: Response,
): Promise<void> => {
  await handleRequest(
    response,
    () => deleteCollar(Number(request.params.id)),
    204,
  );
};
