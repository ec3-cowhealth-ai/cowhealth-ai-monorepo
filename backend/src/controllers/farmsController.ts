...
export const showFarm = async (
  request: Request,
  response: Response,
): Promise<void> => {
  await handleRequest(
    response,
    () => getFarmById(Number(request.params.id)),
    200,
    404,
  );
};

export const storeFarm = async (
  request: Request,
  response: Response,
): Promise<void> => {
...
export const updateFarmController = async (
  ...
  await handleRequest(response, () =>
    updateFarm(Number(request.params.id), request.body),
  );
};

export const destroyFarm = async (
  request: Request,
  response: Response,
): Promise<void> => {
  await handleRequest(
    response,
    () => deleteFarm(Number(request.params.id)),
    204,
  );
};
