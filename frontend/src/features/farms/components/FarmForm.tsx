  ...
  return (
    ...
    >
      <div className="form-field">
        ...
        {errors.name && <span style={{ color: "var(--danger)", fontSize: "var(--t-sm)" }}>{errors.name.message}</span>}
      </div>

      <div className="form-field">
        <label className="form-field__label is-required">CNPJ</label>
        <input type="text" className="form-field__input" placeholder="00.000.000/0000-00" {...register("cnpj")} />
        {errors.cnpj && <span style={{ color: "var(--danger)", fontSize: "var(--t-sm)" }}>{errors.cnpj.message}</span>}
      </div>

      <div className="form-field">
        <label className="form-field__label is-required">Endereço</label>
        <input type="text" className="form-field__input" {...register("address")} />
        {errors.address && <span style={{ color: "var(--danger)", fontSize: "var(--t-sm)" }}>{errors.address.message}</span>}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--s-3)" }}>
        <div className="form-field">
          ...
          {errors.city && <span style={{ color: "var(--danger)", fontSize: "var(--t-sm)" }}>{errors.city.message}</span>}
        </div>

        <div className="form-field">
          <label className="form-field__label is-required">Estado</label>
          <input type="text" className="form-field__input" placeholder="SP" maxLength={2} {...register("state")} />
          {errors.state && <span style={{ color: "var(--danger)", fontSize: "var(--t-sm)" }}>{errors.state.message}</span>}
        </div>
      </div>
      ...
      <div className="form-field">
        ...
        {errors.phone && <span style={{ color: "var(--danger)", fontSize: "var(--t-sm)" }}>{errors.phone.message}</span>}
      </div>

      <div className="form-field">
        <label className="form-field__label is-required">Email</label>
        <input type="email" className="form-field__input" {...register("email")} />
        {errors.email && <span style={{ color: "var(--danger)", fontSize: "var(--t-sm)" }}>{errors.email.message}</span>}
      </div>
    </FormModal>
  );
};
