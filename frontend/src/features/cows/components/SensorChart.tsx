...
export const SensorChart = ({
  ...
  const minValue = data.length > 0
    ? Math.min(...data.map((p) => p.average)).toFixed(1)
    : "N/A";

  const maxValue = data.length > 0
    ? Math.max(...data.map((p) => p.average)).toFixed(1)
    : "N/A";
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "var(--s-2)" }}>
        <div className="kpi-card">
          ...
          <p className="kpi-card__value">
            {minValue}<span className="kpi-card__unit">{unit}</span>
          </p>
          {minThreshold && (
        ...
        <div className="kpi-card">
          ...
          <p className="kpi-card__value">
            {avgValue}<span className="kpi-card__unit">{unit}</span>
          </p>
        </div>
        <div className="kpi-card">
          <p className="kpi-card__label">Maximo</p>
          <p className="kpi-card__value">
            {maxValue}<span className="kpi-card__unit">{unit}</span>
          </p>
          {maxThreshold && (
