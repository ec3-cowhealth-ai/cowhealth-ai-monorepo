...
function PermissionFormModal({ open, permission, onClose, isLoading, onSubmit }: PermissionFormModalProps) {
      ...
      <div style={{ marginTop: "var(--s-4)" }}>
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="loading-skeleton"
            style={{ height: 48, marginBottom: "var(--s-2)", borderRadius: "var(--r-md)" }} />
        ))}
      </div>
    );
  }

  if (isError) return <ErrorState title="Erro ao carregar permissoes" description="Nao foi possivel carregar a lista." />;
    ...
    <div style={{ marginTop: "var(--s-4)", display: "flex", flexDirection: "column", gap: "var(--s-3)" }}>
      ...
      <div style={{ display: "flex", gap: "var(--s-3)", alignItems: "center" }}>
        <div className="form-field" style={{ flex: 1, margin: 0 }}>
          <input className="form-field__input" placeholder="Buscar permissao..."
            ...
            <tbody>
              {filtered.map((perm) => (
                <tr key={perm.id}>
                  ...
                  <td style={{ color: "var(--text-secondary)", fontSize: "var(--t-sm)" }}>
                  ...
                  <td>
                    <div style={{ display: "flex", gap: "var(--s-2)", justifyContent: "flex-end" }}>
                      ...
                      <button className="btn btn-sm btn-danger" onClick={() => setDeletingPerm(perm)}>
                        Excluir
                      </button>
                    </div>
                  </td>
      ...
      <ConfirmDialog
        ...
        isDangerous
        onConfirm={() => deletePermission(String(deletingPerm!.id), { onSuccess: () => setDeletingPerm(null) })}
