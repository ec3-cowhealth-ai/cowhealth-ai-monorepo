...
const kpis = [
  { label: "Saúde do rebanho", value: "84", unit: "/100", sub: "", delta: "↑ 6 vs. últimos 7 dias", tone: "good" as const, icon: "shield" as const },
  { label: "Vacas em risco",    value: "18", unit: "",     sub: "7,3% do rebanho",  delta: "↑ 3 vs. últimos 7 dias", tone: "warn" as const, icon: "heart" as const },
  { label: "Recém-paridas",     value: "24", unit: "",     sub: "9,7% do rebanho",  delta: "↓ 2 vs. últimos 7 dias", tone: "good" as const, icon: "cow"   as const },
  { label: "Em aberto",         value: "32", unit: "",     sub: "13,0% do rebanho", delta: "↑ 1 vs. últimos 7 dias", tone: "good" as const, icon: "circle" as const },
  { label: "Temp. média",       value: "38,6", unit: "°C", sub: "",                 delta: "↑ 0,2°C vs. últimos 7 dias", tone: "warn" as const, icon: "temp" as const },
];
...
const alertFeed: { id: string; title: string; cow: string; time: string; sev: Severity; icon: "temp" | "heart" | "activity" | "clock" }[] = [
  ...
  { id: "2", title: "Freq. cardíaca elevada",    cow: "Vaca 1247", time: "há 25 min", sev: "Médio", icon: "heart"    },
  ...
  { id: "5", title: "Freq. cardíaca elevada",    cow: "Vaca 0456", time: "há 3 h",    sev: "Médio", icon: "heart"    },
...
export const DashboardPreviewPage = () => {
  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", background: C.bg, minHeight: "100vh", color: C.text }}>
      ...
      <div style={{
        background: "#1a1f1c", color: "#f5f1ea", padding: "10px 24px",
        display: "flex", alignItems: "center", justifyContent: "space-between",
        fontSize: 13, position: "sticky", top: 0, zIndex: 100,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ color: "#d9e4dc" }}>
            Painel de referência visual — dados mockados para guiar o desenvolvimento do dashboard autenticado.
          </span>
        </div>
        <button
          onClick={() => navigate("/")}
          style={{
            ...
            color: "#f5f1ea", borderRadius: 8, padding: "5px 14px", fontSize: 12,
  ...
  return (
    <aside style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32, padding: "0 8px" }}>
        <div style={{
          width: 40, height: 40, borderRadius: 12, background: C.sidebarActive,
          display: "flex", alignItems: "center", justifyContent: "center",
        }}>
          <CowGlyph style={{ width: 22, height: 22, color: "#9bd3a8" }} />
        </div>
        <div>
          <div style={{ fontWeight: 600, color: "#fff", letterSpacing: "-0.02em", fontSize: 14 }}>CowHealth AI</div>
        ...
        {navItems.map((n) => (
          <button
            ...
            style={{
              ...
              padding: "10px 12px", borderRadius: 8, fontSize: 13, border: "none", cursor: "pointer",
      ...
      {/* Help */}
      <div style={{ marginTop: 24, borderRadius: 12, background: C.sidebarActive, padding: 16, fontSize: 13 }}>
      ...
      {/* User */}
      <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 12, padding: "0 8px" }}>
        <div style={{
          width: 40, height: 40, borderRadius: 999, background: C.sidebarActive,
          display: "flex", alignItems: "center", justifyContent: "center",
...
function KpiCard({ label, value, unit, sub, delta, tone, icon, index }: typeof kpis[number] & { index: number }) {
...
function CowProfile() {
  return (
    <section style={{ ...card, gridColumn: "span 3" }}>
      <div style={{ position: "relative", width: 160, height: 160, margin: "0 auto", borderRadius: 999, overflow: "hidden", border: `4px solid ${C.bg}`, boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
        <img src={cowImg} alt="Vaca 1247" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        <button style={{
      ...
      <div style={{ textAlign: "center", marginTop: 16 }}>
        <div style={{ fontFamily: "'Instrument Serif', Georgia, serif", fontSize: 28, color: C.text, fontWeight: 400 }}>Vaca 1247</div>
        <div style={{ fontSize: 11, color: C.muted, marginTop: 4, letterSpacing: "0.08em", textTransform: "uppercase" }}>
        ...
        {profileRows.map(([k, v]) => (
          <div key={k} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 13 }}>
            <dt style={{ color: C.muted, display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ width: 12, height: 12, borderRadius: 999, background: "#e6f1ea", display: "inline-block" }} />
              {k}
            </dt>
  ...
  return (
    <section style={{ gridColumn: "span 6", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={card}>
        {/* Tabs */}
        <div style={{
          display: "flex", gap: 24, borderBottom: `1px solid ${C.border}`,
          margin: "-20px -20px 0", padding: "0 20px",
        }}>
          {tabs.map((t, i) => (
            <button key={t} style={{
              padding: "10px 0", fontSize: 13, border: "none", background: "transparent",
              cursor: "pointer", borderBottom: i === 0 ? `2px solid ${C.green}` : "2px solid transparent",
              color: i === 0 ? C.green : C.muted, fontWeight: i === 0 ? 600 : 400,
              marginBottom: -1,
            }}>{t}</button>
          ))}
        </div>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 20, fontWeight: 600, color: C.orange }}>39,1 <span style={{ fontSize: 11 }}>°C</span></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={card}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
            ...
            <div style={{ textAlign: "right" }}>
              <div><span style={{ fontSize: 20, fontWeight: 600 }}>72</span> <span style={{ fontSize: 11, color: C.muted }}>bpm</span></div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <ul style={{ listStyle: "none", margin: 0, padding: 0, flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                <li key={f.label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 11 }}>
    ...
    <aside style={{ gridColumn: "span 3", display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={card}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          ...
          <button style={{ fontSize: 12, color: C.green, fontWeight: 500, background: "none", border: "none", cursor: "pointer" }}>
            Ver tudo
          </button>
        </div>
      ...
      {/* Promo card */}
      <div style={{
        borderRadius: 16, background: C.sidebar, color: "#fff", padding: 20, position: "relative", overflow: "hidden",
      }}>
        ...
        <button style={{
          ...
          background: "#fff", color: C.sidebar, fontSize: 12, fontWeight: 600,
        ...
        <div style={{
          position: "absolute", right: -12, bottom: -12, width: 90, height: 120,
          borderRadius: 16, background: C.sidebarActive, border: `4px solid ${C.sidebar}`,
  ...
  return (
    <section style={card}>
      ...
      <div style={{ marginTop: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", fontSize: 11, color: C.muted, marginTop: 8 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 16, marginTop: 24 }}>
          {timeline.map((t, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: "#e6f1ea", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
...
function Sparkline({ color, variant }: { color: string; variant: "a" | "b" }) {
  ...
  return (
    <svg viewBox="0 0 100 24" preserveAspectRatio="none" style={{ width: "100%", height: "100%" }}>
      <path d={variant === "a" ? a : b} fill="none" stroke={color} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
    ...
    <div style={{ marginTop: 16 }}>
      <div style={{ position: "relative", height: 160 }}>
        <svg viewBox="0 0 100 90" preserveAspectRatio="none" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}>
          ...
          <line x1="0" y1="20" x2="100" y2="20" stroke={C.red}  strokeWidth="0.4" strokeDasharray="2 2" />
          <line x1="0" y1="75" x2="100" y2="75" stroke="#3b6fa0" strokeWidth="0.4" strokeDasharray="2 2" />
        ...
        <div style={{ position: "absolute", top: 0, right: 0, fontSize: 10, color: C.red }}>39,5 Limite de febre</div>
        <div style={{ position: "absolute", bottom: 12, right: 0, fontSize: 10, color: "#3b6fa0" }}>37,5 Limite mínimo</div>
        <div style={{ position: "absolute", left: 0, top: 0, height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", fontSize: 10, color: C.muted, paddingRight: 4 }}>
          <span>40,0</span><span>39,0</span><span>38,0</span><span>37,0</span>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", fontSize: 10, color: C.muted, marginTop: 4 }}>
...
function HeartChart() {
  ...
  return (
    <div style={{ marginTop: 12, height: 90, position: "relative" }}>
      ...
      <div style={{ position: "absolute", left: 0, bottom: 16, fontSize: 10, color: C.muted }}>40</div>
  ...
  return (
    <div style={{ position: "relative", width: 90, height: 90, flexShrink: 0 }}>
      <svg viewBox="0 0 80 80" style={{ width: "100%", height: "100%", transform: "rotate(-90deg)" }}>
        ...
        <circle cx="40" cy="40" r={r} stroke={C.green} strokeWidth="7" fill="none"
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: C.green, fontWeight: 500, marginTop: 4 }}>Risco baixo</div>
    </div>
  );
}
...
function CheckDot({ ok }: { ok: boolean }) {
  ...
  return (
    <span style={{ width: 14, height: 14, borderRadius: 999, background: bg, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
      <svg viewBox="0 0 10 10" style={{ width: 8, height: 8 }}>
        <path d="M2 5 L4 7 L8 3" stroke={color} strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </span>
  );
}
...
function NavIcon({ name }: { name: string }) {
  ...
  switch (name) {
    case "grid":   return <svg style={s} viewBox="0 0 20 20" fill="currentColor"><path d="M3 3h6v6H3zM11 3h6v6h-6zM3 11h6v6H3zM11 11h6v6h-6z"/></svg>;
    case "cow":    return <CowGlyph style={s} />;
    case "bell":   return <svg style={s} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M5 9a5 5 0 0110 0v3l1 2H4l1-2V9zM8 16a2 2 0 004 0"/></svg>;
    case "heart":  return <svg style={s} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M10 16s-6-3.5-6-8a3.5 3.5 0 016-2.5A3.5 3.5 0 0116 8c0 4.5-6 8-6 8z"/></svg>;
    case "report": return <svg style={s} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><rect x="4" y="3" width="12" height="14" rx="1.5"/><path d="M7 7h6M7 10h6M7 13h4"/></svg>;
    case "drop":   return <svg style={s} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M10 3c2 4 5 6 5 9a5 5 0 01-10 0c0-3 3-5 5-9z"/></svg>;
    case "spark":  return <svg style={s} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M10 3v4M10 13v4M3 10h4M13 10h4"/></svg>;
    case "box":    return <svg style={s} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M3 7l7-4 7 4v6l-7 4-7-4V7zM3 7l7 4 7-4M10 11v8"/></svg>;
    case "gear":   return <svg style={s} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><circle cx="10" cy="10" r="2.5"/><path d="M10 2v2M10 16v2M2 10h2M16 10h2M4.5 4.5l1.5 1.5M14 14l1.5 1.5M15.5 4.5L14 6M6 14l-1.5 1.5"/></svg>;
    default:       return null;
  }
}

function KpiIcon({ name, color }: { name: string; color: string }) {
  const p = { style: { width: 18, height: 18, color }, fill: "none" as const, stroke: "currentColor" as const, strokeWidth: 1.6 };
  switch (name) {
    case "shield": return <svg {...p} viewBox="0 0 20 20"><path d="M10 2l6 2v5c0 4-3 7-6 9-3-2-6-5-6-9V4l6-2z"/><path d="M7 10l2 2 4-4"/></svg>;
    case "heart":  return <svg {...p} viewBox="0 0 20 20"><path d="M10 16s-6-3.5-6-8a3.5 3.5 0 016-2.5A3.5 3.5 0 0116 8c0 4.5-6 8-6 8z"/></svg>;
    case "cow":    return <CowGlyph style={{ width: 18, height: 18, color }} />;
    case "circle": return <svg {...p} viewBox="0 0 20 20"><circle cx="10" cy="10" r="6"/><path d="M10 4v3M10 13v3M4 10h3M13 10h3"/></svg>;
...
function AlertIcon({ name, color }: { name: string; color: string }) {
  const p = { style: { width: 16, height: 16, color }, fill: "none" as const, stroke: "currentColor" as const, strokeWidth: 1.6 };
  switch (name) {
    case "temp":     return <svg {...p} viewBox="0 0 20 20"><path d="M10 3a2 2 0 012 2v7a3 3 0 11-4 0V5a2 2 0 012-2z"/></svg>;
    case "heart":    return <svg {...p} viewBox="0 0 20 20"><path d="M10 16s-6-3.5-6-8a3.5 3.5 0 016-2.5A3.5 3.5 0 0116 8c0 4.5-6 8-6 8z"/></svg>;
...
function TimelineIcon({ name }: { name: string }) {
  const p = { style: { width: 16, height: 16, color: C.green }, fill: "none" as const, stroke: "currentColor" as const, strokeWidth: 1.6 };
  switch (name) {
    case "heart": return <svg {...p} viewBox="0 0 20 20"><path d="M10 16s-6-3.5-6-8a3.5 3.5 0 016-2.5A3.5 3.5 0 0116 8c0 4.5-6 8-6 8z"/></svg>;
    case "cow":   return <CowGlyph style={{ width: 16, height: 16, color: C.green }} />;
    case "alert": return <svg {...p} viewBox="0 0 20 20"><path d="M10 3l8 14H2L10 3z"/><path d="M10 9v3M10 14v.5"/></svg>;
    default:      return null;
  }
}

function TempIcon({ style }: SvgProps)    { return <svg style={style} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M10 3a2 2 0 012 2v7a3 3 0 11-4 0V5a2 2 0 012-2z"/></svg>; }
function HeartIcon({ style }: SvgProps)   { return <svg style={style} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M10 16s-6-3.5-6-8a3.5 3.5 0 016-2.5A3.5 3.5 0 0116 8c0 4.5-6 8-6 8z"/></svg>; }
function ShieldIcon({ style }: SvgProps)  { return <svg style={style} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M10 2l6 2v5c0 4-3 7-6 9-3-2-6-5-6-9V4l6-2z"/></svg>; }
function CameraIcon({ style }: SvgProps)  { return <svg style={style} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><rect x="3" y="6" width="14" height="10" rx="2"/><circle cx="10" cy="11" r="2.5"/><path d="M7 6l1-2h4l1 2"/></svg>; }
function SpeechIcon({ style }: SvgProps)  { return <svg style={style} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M4 5h12v8H9l-3 3v-3H4z"/></svg>; }
function CalIcon({ style }: SvgProps)     { return <svg style={style} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><rect x="3" y="5" width="14" height="12" rx="1.5"/><path d="M3 8h14M7 3v4M13 3v4"/></svg>; }
function FilterIcon({ style }: SvgProps)  { return <svg style={style} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M3 5h14M5 10h10M8 15h4"/></svg>; }
function ChevronDown({ style }: SvgProps) { return <svg style={style} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M5 8l5 5 5-5"/></svg>; }
function ChevronRight({ style }: SvgProps){ return <svg style={style} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M8 5l5 5-5 5"/></svg>; }
function ArrowUpRight({ style }: SvgProps){ return <svg style={style} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M6 14L14 6M8 6h6v6"/></svg>; }
...
const btnOutline: React.CSSProperties = {
  ...
  cursor: "pointer", color: C.text, boxShadow: "0 1px 2px rgba(0,0,0,0.04)",
};
