...
export function CowSelectorBar({
  ...
  const alertCow = mode === "alert"
    ? cowList.find((c) => String(c.id) === selectedCowId)
    : null;
  ...
  return (
    <div style={{ ...cardStyle }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        <span style={{ fontSize: 12, color: C.muted, fontWeight: 500, marginRight: 4 }}>Seleção de vaca:</span>
      ...
      {mode === "farm" && (
        <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ position: "relative" }}>
            ...
            >
              <option value="" disabled>Selecionar fazenda…</option>
              {farms.map((f) => (
      ...
      {(mode === "farm" || mode === "global") && (
        <div style={{
          ...
          {isLoadingCows ? (
            ...
            <span style={{ fontSize: 12, color: C.muted }}>
              {mode === "farm" && !selectedFarmId ? "Selecione uma fazenda para ver as vacas." : "Nenhuma vaca encontrada."}
            </span>
          ) : (
            cowList.map((cow) => {
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background:   selected ? "#e6f1ea" : "#fff",
                    cursor:       "pointer",
                    fontSize:     12,
      ...
      {mode === "alert" && (
        <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 10, fontSize: 13 }}>
          {selectedCowId ? (
            <>
              <span style={{ width: 8, height: 8, borderRadius: 999, background: C.orange, flexShrink: 0 }} />
              <span style={{ color: C.text }}>
                Selecionada automaticamente:{" "}
                <strong>{alertCow ? `${alertCow.tag}${alertCow.name !== alertCow.tag ? ` — ${alertCow.name}` : ""}` : `ID ${selectedCowId}`}</strong>
              </span>
