      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        ...
        <div style={{ textAlign: "right" }}>
          {!cowId ? (
            ...
            <>
              <div style={{ fontSize: 20, fontWeight: 600, color: latest && latest >= 39.5 ? C.orange : C.text }}>
    ...
    <div style={{ marginTop: 16 }}>
      <ResponsiveContainer width="100%" height={160}>
        <AreaChart data={chartData} margin={{ top: 10, right: 8, bottom: 0, left: -20 }}>
          ...
          <Area type="monotone" dataKey="temp" stroke={C.green} strokeWidth={1.5} fill="url(#tempGrad)" dot={false} activeDot={{ r: 4, fill: C.green }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
        ...
        <div style={{ marginTop: 12 }}>
          <ResponsiveContainer width="100%" height={90}>
            <LineChart data={data.map((p) => ({ date: p.date, bpm: Number(p.average.toFixed(0)) }))} margin={{ top: 4, right: 4, bottom: 0, left: -28 }}>
              ...
              <Tooltip contentStyle={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 12 }} formatter={(v: number) => [`${v} bpm`, "Média"]} />
...
function ComingSoon({ label }: { label: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 200, gap: 8 }}>
      <div style={{ width: 40, height: 40, borderRadius: 999, background: "#e6f1ea", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <svg viewBox="0 0 20 20" style={{ width: 18, height: 18, color: C.green }} fill="none" stroke="currentColor" strokeWidth="1.6">
          <circle cx="10" cy="10" r="6"/><path d="M10 7v3l2 1"/>
        </svg>
      </div>
...
function ChartSkeleton({ height }: { height: number }) {
  return (
    <div style={{ height, marginTop: 16, borderRadius: 8, background: "#f0ece4", animation: "pulse 1.5s ease-in-out infinite" }} />
  );
}

function ChartEmpty({ height }: { height: number }) {
  return (
    <div style={{ height, marginTop: 16, display: "flex", alignItems: "center", justifyContent: "center", border: `1px dashed ${C.border}`, borderRadius: 8 }}>
